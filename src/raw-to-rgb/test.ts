...
test("create_raw_to_rgb", ({ eq }) => {
  ...
  eq(rawToRgb({ ranges: [[0, 65536], [2314, 2314]], new_no_data_value: 255 })([1275, 2314]), [5, 254, 0]);
  eq(rawToRgb({ ranges: [[0, 65536], [2314, 2314]], new_no_data_value: 0 })([1275, 2314]), [6, 255, 0]);
  eq(rawToRgb({ ranges: [[0, 65536], [2314, 2314]], new_no_data_value: 255, flip: true })([1275, 2314]), [249, 254, 0]);
});